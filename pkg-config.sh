...
# cd to the idpe directory first to ensure we build the version of pkg-config specified in idpe.
# Go runs pkg-config from the directory of the package it's trying to configure,
# so without cd-ing to idpe first, we would pick up flux's go.mod and build the version of pkg-config specified there.
(cd "${ROOTIDPE_DIR}" && "${GO}" build -o ${tmpdir}/pkg-config github.com/influxdata/pkg-config) &> ${tmpdir}/go_build_output
if [ "$?" -ne 0 ]; then
